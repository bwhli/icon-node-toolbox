...
GCP_API_REQUEST=$(gcloud compute instances list --filter="zone ~ $REGION_ID AND tags.items=(ctz-node-primary,ctz-node-backup)" --format="json(name,networkInterfaces.[].networkIP)")
...
CURRENT_HAPROXY_IPS=$(cat $HAPROXY_CONFIG_DIR/haproxy.cfg | egrep -o '10\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}\.[[:digit:]]{1,3}' | sort -n | tr '\n' ' ')
...
if [[ $CURRENT_HAPROXY_IPS != $GCP_INTERNAL_IPS ]]; then
  ...
  for i in $( seq 0 $(($GCP_API_REQUEST_ARRAY_LENGTH - 1)) ); do
    ...
    if [ $HEALTH_CHECK_REQUEST == "200" ]; then
      echo "$GCP_INSTANCE_NAME ($GCP_INSTANCE_INTERNAL_IP) is returning 200 response. Adding to HAProxy config..."
  ...
  rm -rf $HAPROXY_CONFIG_DIR/haproxy.cfg.bkp
fi
